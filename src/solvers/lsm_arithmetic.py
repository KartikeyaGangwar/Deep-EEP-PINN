...
class LongstaffSchwartzArithmeticSolver:
    ...
    def compute_arithmetic_payoff(self, S_t: np.ndarray) -> np.ndarray:
        """
        Computes exact arithmetic basket payoff: max(K - sum(w_i * S_i), 0).
        """
    ...
    def price(self, S0: np.ndarray = None):
        ...
        for k in range(self.num_steps - 1, 0, -1):
            ...
            if np.sum(itm_mask) > 30:
                ...
                B_itm = S_k[itm_mask] @ self.weights
                
                A = self.build_quadratic_basis(S_k[itm_mask], B_itm)
