"""
10-Asset Master Benchmark Pipeline: High-Dimensional Multi-Asset American Basket Options (d=10).
Compares:
...
from src.evaluation.visualizer_10d import (
    ...
    plot_10d_premium_slice,
    export_10d_latex_table
)

def run_10d_master_benchmark():
    print("=" * 85)
    print("   INSTITUTIONAL 10-ASSET AMERICAN BASKET OPTION BENCHMARK (d=10, 100k LSM paths)")
    print("=" * 85)
    ...
    # -------------------------------------------------------------
    print("\n>> Computing 10D Early-Exercise Premium Manifold Slice for Publication...")
    n_slice = 60
    ...
    for j in range(2, cfg.d):
        slice_points[:, j] = 100.0
    ...
    # -------------------------------------------------------------
    print("\n>> Generating High-Resolution Phase 3 (10-Asset) Publication Figures & LaTeX Table...")
    ...
    plot_10d_premium_slice(S1_grid, S2_grid, e_slice,
                           save_path="figures/phase3_10d_geom_premium_surface_slice.png")
    export_10d_latex_table(results_summary,
                           save_path="figures/phase3_10d_geom_benchmark_summary_table.tex")
    ...
    save_multid_csv(results_summary, csv_path="results/phase3_10d_results.csv", summary_stats=summary_stats)
    append_to_master_json("phase3_10d_geometric_basket", {
        "pricing_table": results_summary,
        "summary_statistics": summary_stats,
        "train_time_seconds": round(pinn_train_time, 2)
    })
    
    print(f">> All Phase 3 (10D) Deliverables (Figures, LaTeX, CSV, JSON) Generated Successfully!")
